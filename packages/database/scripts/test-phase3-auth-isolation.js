...
const { checkAndIncrementRateLimit, hashRateLimitKey } = require("../dist/rate-limiter.js");
...
async function run20CaseAuthenticationSecurityMatrix() {
  ...
  const authBad = await verifyPasswordlessToken(
    prisma,
    userAEmail,
  ...
  if (authExpired.success !== false || authExpired.error !== "TOKEN_EXPIRED")
  ...
  const successes = concResults.filter(function (r) {
    return r.success;
  });
  const failures = concResults.filter(function (r) {
    return !r.success;
  });
  ...
  await prisma.session.create({
    data: {
      userId: user.id,
      sessionToken: expiredSessionHash,
  ...
  if ((await resolveHashedServerSession(prisma, sToken)) !== null)
  ...
  if (process.env.NODE_ENV === "production" && prodDelivery.provider === "TEST_MOCK")
  ...
  if (blocked.allowed !== false)
    throw new Error("Test 19 Failed: 6th request was not rate-limited");
