import { ConfidenceInput, ConfidenceExplanation, EvidenceSignalItem, ClaimType } from "./types.js";

export const CONFIDENCE_RUBRIC_VERSION = "2.0.0";

const CRITICAL_CLAIMS: ClaimType[] = [
  "PAIN_EXISTENCE",
  "BUYER_IDENTITY",
  "BUYER_DEMAND",
  "WILLINGNESS_TO_PAY",
...
export function calculateEvidenceConfidence(
  ...
  const signalMap = new Map<string, EvidenceSignalItem>();
  ...
  const evidenceVolumeScore = Math.min(
    15,
    Math.round(15 * (Math.log(1 + nVerified) / Math.log(1 + 15))),
  );
  ...
  const uniqueFamilies = new Set(
    uniqueSignals.map((s) => s.sourceFamily || "COMMUNITY").filter(Boolean),
  );
  ...
  const buyerIntentSignals = uniqueSignals.filter(
    (s) =>
      s.signalType === "PURCHASE_INTENT" ||
  ...
  for (const s of uniqueSignals) {
    if (!s.publishedAt || s.publishedAtPrecision === "UNKNOWN") {
      ...
      if (ageDays <= 90) totalRecencyWeight += 1.0;
      else if (ageDays <= 180) totalRecencyWeight += 0.75;
      else if (ageDays <= 365) totalRecencyWeight += 0.4;
      else totalRecencyWeight += 0.15;
    }
  }
  ...
  const coveredSupportingClaims = new Set<ClaimType>();
  for (const l of validLinks) {
  ...
  const contradictingGroups = new Set(
    contradictingLinks.map((l) => l.signal?.independenceKey || `id:${l.normalizedSignalId}`),
  );
  const supportingGroups = new Set(
    supportingLinks.map((l) => l.signal?.independenceKey || `id:${l.normalizedSignalId}`),
  );
  ...
  if (buyerIntentSignals.length >= 2) strongestArea = "Direct Buyer Intent & WTP";
  else if (
    coveredSupportingClaims.has("PAIN_EXISTENCE") &&
  ...
  const contradictionNotes = contradictingLinks.map(
    (l) => l.explanation || `Contradicts ${l.claimType} with ${l.supportStrength} strength`,
  );
