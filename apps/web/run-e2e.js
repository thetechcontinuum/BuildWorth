...
function fetchUrl(urlPath) {
  return new Promise((resolve, reject) => {
    http.get("http://localhost:3009" + urlPath, (res) => {
      let data = "";
...
async function run() {
  ...
  try {
    ...
    try {
      ...
      captureScreenshot("/opportunities/b2b-saas-audit-readiness-automation", "01_desktop_executive_decision_summary.png", 1280, 900);
      console.log("PASS 1: Executive Decision Summary renders verified recommendation and key metrics");
      passed++;
    } catch (e) {
    ...
    try {
      const res = await fetchUrl("/opportunities/b2b-saas-audit-readiness-automation");
      if (!res.data.includes("Target Customer &amp; Buyer Segments") && !res.data.includes("Target Customer & Buyer Segments")) {
        throw new Error("Missing customer segments section");
      }
      captureScreenshot("/opportunities/b2b-saas-audit-readiness-automation", "02_desktop_customer_segments.png", 1280, 1100);
      console.log("PASS 2: Structured Customer Segments renders role distinctions and buying triggers");
      passed++;
    } catch (e) {
    ...
    try {
      ...
      captureScreenshot("/opportunities/b2b-saas-audit-readiness-automation", "03_desktop_narrow_mvp_scope.png", 1280, 1000);
    ...
    try {
      ...
      captureScreenshot("/opportunities/b2b-saas-audit-readiness-automation", "04_desktop_cost_benefit_economics.png", 1280, 1100);
    ...
    try {
      const res = await fetchUrl("/opportunities/b2b-saas-audit-readiness-automation");
      if (!res.data.includes("Competition &amp; Asymmetric Wedge") && !res.data.includes("Competition & Asymmetric Wedge")) {
        throw new Error("Missing competition & asymmetric wedge section");
      }
      captureScreenshot("/opportunities/b2b-saas-audit-readiness-automation", "05_desktop_competition_wedge.png", 1280, 1000);
    ...
    try {
      const res = await fetchUrl("/opportunities/b2b-saas-audit-readiness-automation");
      if (!res.data.includes("Risk Register &amp; Assumption Matrix") && !res.data.includes("Risk Register & Assumption Matrix")) {
        throw new Error("Missing risk register and assumption matrix");
      }
      captureScreenshot("/opportunities/b2b-saas-audit-readiness-automation", "06_desktop_risks_and_assumptions.png", 1280, 1000);
    ...
    try {
      const res = await fetchUrl("/opportunities/b2b-saas-audit-readiness-automation");
      if (!res.data.includes("Validation Experiments Roadmap") || !res.data.includes("Kill Criterion")) {
        throw new Error("Missing validation experiments roadmap");
      }
      captureScreenshot("/opportunities/b2b-saas-audit-readiness-automation", "07_desktop_validation_roadmap.png", 1280, 1000);
      console.log("PASS 7: Validation Experiments Roadmap renders prioritized experiments with kill criteria");
      passed++;
    } catch (e) {
    ...
    try {
      captureScreenshot("/opportunities/b2b-saas-audit-readiness-automation", "08_mobile_decision_blueprint.png", 390, 844);
      console.log("PASS 8: Mobile viewport (390x844) renders decision blueprint without horizontal overflow");
      passed++;
    } catch (e) {
    ...
    console.log(`=== Phase 2 Browser E2E Results: ${passed} Passed, ${failed} Failed (Exit code ${failed === 0 ? 0 : 1}) ===`);
