...
export function CostBenefitEconomicsSection({ blueprint }: Props) {
  ...
  const scenario =
    blueprint.financialScenarios.find((s) => s.scenarioType === selectedScenario) ||
    blueprint.financialScenarios[0];
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-800 pb-4">
        <div>
          ...
          <p className="text-xs text-zinc-400 mt-1">
            Authoritative integer cents arithmetic with explicit scenario models and customer ROI
            quantification.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 bg-zinc-950/80 border border-zinc-800/80 rounded-xl">
        <div className="space-y-1">
          ...
          <div className="text-lg font-bold text-zinc-100">
            `$${(scenario?.monthlyPriceCents || 0) / 100} / mo`
          </div>
          <div className="text-[10px] text-zinc-400">
            `Var Cost: $${(scenario?.variableCostPerCustomerCents || 0) / 100}/cust`
          </div>
        </div>
        ...
        <div className="space-y-1">
          ...
          <div className="text-lg font-bold text-emerald-400">
            {scenario?.grossMarginPercent.status === "CALCULATED"
              ? `${scenario.grossMarginPercent.value}%`
              : "N/A"}
          </div>
        ...
        <div className="space-y-1">
          ...
          <div className="text-lg font-bold text-zinc-100">
            {scenario?.breakEvenCustomers.status === "CALCULATED"
              ? `${scenario.breakEvenCustomers.value} cust`
              : "Negative"}
          </div>
          <div className="text-[10px] text-zinc-400">
            `Fixed Cost: $${(scenario?.monthlyFixedCostCents || 0) / 100}/mo`
          </div>
        </div>
        ...
        <div className="space-y-1">
          ...
          <div className="text-lg font-bold text-indigo-300">
            {scenario?.customerRoiPercent.status === "CALCULATED"
              ? `${scenario.customerRoiPercent.value}%`
              : "N/A"}
          </div>
          <div className="text-[10px] text-zinc-400">
            `Payback: ${scenario?.customerPaybackMonths.value} mos`
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {buildCosts.map((c) => (
              <div
                key={c.id}
                className="p-3 bg-zinc-950/50 border border-zinc-800/60 rounded-lg flex items-center justify-between text-xs"
              >
                <div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-mono font-bold text-emerald-300">Annual Value</span>
          </div>
          <div className="space-y-2">
            {benefitDrivers.map((b) => (
              <div
                key={b.id}
                className="p-3 bg-zinc-950/50 border border-emerald-950/40 rounded-lg space-y-1 text-xs"
              >
                <div className="flex items-center justify-between">
