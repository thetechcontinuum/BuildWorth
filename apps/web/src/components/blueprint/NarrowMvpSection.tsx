...
export function NarrowMvpSection({ features }: Props) {
  ...
  return (
    <section id="section-narrow-mvp" className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 sm:p-8 space-y-6 shadow-lg">
      ...
      <div className="space-y-3">
        <h3 className="text-xs font-mono text-emerald-400 uppercase tracking-wider flex items-center gap-1.5">
          <span className="w-2 h-2 rounded-full bg-emerald-400"></span> Core Must-Have Scope (Delivery Milestone 1)
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {mustHave.map((f) => (
            <div key={f.id} className="bg-zinc-950/70 border border-emerald-900/40 rounded-xl p-4 space-y-2.5">
                ...
                <div className="pt-2 border-t border-zinc-800/60">
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {f.requiredIntegrations.map((it, i) => (
                      <span key={i} className="text-[10px] font-mono px-1.5 py-0.5 bg-zinc-900 text-zinc-300 rounded border border-zinc-800">
                        {it}
                      </span>
                    ))}
                  </div>
      ...
      {later.length > 0 && (
        <div className="space-y-3 pt-2">
          <h3 className="text-xs font-mono text-zinc-400 uppercase tracking-wider flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-zinc-500"></span> Deferred to Post-MVP (Milestone 2+)
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {later.map((f) => (
              <div key={f.id} className="bg-zinc-950/40 border border-zinc-800/80 rounded-xl p-3.5 space-y-1">
