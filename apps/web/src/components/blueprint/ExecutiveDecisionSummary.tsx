...
export function ExecutiveDecisionSummary({
  ...
  const { decisionEvaluation, financialScenarios, customerSegments, risks, validationExperiments } = blueprint;
  const baseScenario = financialScenarios.find(s => s.scenarioType === "BASE") || financialScenarios[0];
  ...
  return (
    <div id="section-executive-summary" className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 sm:p-8 space-y-6 shadow-xl">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-6 border-b border-zinc-800/80">
        ...
        {/* Big Decision Card */}
        <div className={`p-4 rounded-xl border ${badge.bg} flex flex-col justify-center min-w-[280px] space-y-1`}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 pt-2">
        ...
        <div className="bg-zinc-950/60 border border-zinc-800/80 p-3.5 rounded-xl space-y-1">
          ...
          <div className="text-[10px] text-zinc-500">Gross Margin {baseScenario?.grossMarginPercent.value ?? 0}%</div>
        </div>

        <div className="bg-zinc-950/60 border border-zinc-800/80 p-3.5 rounded-xl space-y-1">
          <div className="text-[11px] font-mono text-zinc-500 uppercase">Break-Even</div>
          <div className="text-base font-bold text-zinc-100">
            {baseScenario?.breakEvenCustomers.value ? `${baseScenario.breakEvenCustomers.value} cust` : "N/A"}
          </div>
        ...
        <div className="bg-zinc-950/60 border border-zinc-800/80 p-3.5 rounded-xl space-y-1">
          ...
          <div className="text-[10px] text-zinc-500 truncate">{primarySegment?.spendingBehavior?.slice(0, 24) || "Direct Auth"}</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
        <div className="p-3.5 rounded-xl bg-zinc-950/40 border border-zinc-800/80 flex items-start gap-3">
          ...
          <div className="space-y-0.5">
            <div className="text-xs font-mono text-zinc-400 uppercase tracking-wider">Riskiest Assumption</div>
            <div className="text-xs text-zinc-200 leading-snug">
              {risks[0]?.description || "Incumbents develop native solution before distribution moat is established."}
            </div>
          </div>
        </div>
        ...
        <div className="p-3.5 rounded-xl bg-zinc-950/40 border border-zinc-800/80 flex items-start gap-3">
          ...
          <div className="space-y-0.5">
            <div className="text-xs font-mono text-zinc-400 uppercase tracking-wider">Cheapest Next Validation Experiment</div>
            <div className="text-xs text-zinc-200 leading-snug">
              {topExperiment?.hypothesis || "Launch landing page test with 14-day money-back guarantee to measure paid pilot conversions."}
            </div>
          </div>
        </div>
      </div>
