...
export function FounderFitDetailPanel({ evaluation, hasProfile = true }: Props) {
  ...
  if (!hasProfile || !evaluation) {
    return (
      <div id="section-founder-fit" className="p-6 rounded-2xl bg-zinc-900/50 border border-zinc-800 space-y-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-sm text-zinc-400">
              Evaluate this venture opportunity against your specific skills, budget, capacity, and market advantages.
            </p>
          </div>
          <a
            href="/onboarding"
  ...
  return (
    <div id="section-founder-fit" data-testid="founder-fit-dimensions" className="p-6 rounded-2xl bg-zinc-900/60 border border-zinc-800 space-y-6">
      {/* Header & Badges */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-zinc-800/80">
        <div className="space-y-1">
          <div className="flex items-center gap-2.5 flex-wrap">
            ...
            <span className={`text-xs font-mono font-semibold px-2.5 py-0.5 rounded-full border ${categoryColor[evaluation.recommendationCategory]}`}>
              {evaluation.recommendationCategory.replace(/_/g, " ")}
            </span>
          </div>
          <p className="text-xs text-zinc-400">
            Deterministic personalized venture compatibility evaluated against your active profile revision.
          </p>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="bg-zinc-950/80 border border-zinc-800 px-4 py-2 rounded-xl text-center">
            <div className="text-[10px] font-mono text-zinc-500 uppercase tracking-wider">Founder Fit</div>
            <div className="text-2xl font-bold text-white font-mono">
              {evaluation.founderFitScore}<span className="text-xs text-zinc-500">/100</span>
            </div>
          </div>

          <div className="bg-zinc-950/80 border border-zinc-800 px-4 py-2 rounded-xl text-center">
            <div className="text-[10px] font-mono text-zinc-500 uppercase tracking-wider">Fit Confidence</div>
          ...
          <div className="bg-zinc-950/80 border border-zinc-800 px-4 py-2 rounded-xl text-center">
            <div className="text-[10px] font-mono text-zinc-500 uppercase tracking-wider">Personalized Rank</div>
      ...
      {evaluation.blockers.length > 0 && (
        <div data-testid="founder-fit-blockers" className="p-5 rounded-xl bg-rose-500/10 border border-rose-500/30 space-y-3">
          <div className="flex items-center justify-between flex-wrap gap-2 pb-2 border-b border-rose-500/20">
            <div className="flex items-center gap-2 text-rose-400 font-bold text-xs">
              <ShieldAlert className="w-4 h-4" />
              <span>
                {evaluation.blockers.filter(b => !b.isRemovable).length} Hard Blocker(s) &amp; {evaluation.blockers.filter(b => b.isRemovable).length} Removable Blocker(s) Identified
              </span>
            </div>
            <span className="text-[11px] font-mono text-rose-300 font-semibold">
              Total Penalty Applied: -{evaluation.penalties.reduce((acc, p) => acc + p.penaltyPoints, 0)} pts
            </span>
          </div>

          <div className="space-y-2 text-xs text-zinc-300">
            {evaluation.blockers.map((b, idx) => (
              <div key={idx} className="p-2.5 rounded-lg bg-zinc-950/60 border border-rose-500/20 space-y-1">
                ...
                {b.suggestedMitigation && (
                  <div className="text-[11px] text-zinc-400">
                    ↳ <span className="text-indigo-400">Suggested Mitigation:</span> {b.suggestedMitigation}
                  </div>
                )}
              </div>
            ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="p-4 rounded-xl bg-zinc-950/60 border border-zinc-800/80 space-y-3">
          ...
          {evaluation.strengths.length === 0 ? (
            <p className="text-xs text-zinc-500">No strong proprietary overlaps recorded in profile.</p>
          ) : (
            <div className="space-y-2 text-xs">
              {evaluation.strengths.map((s, idx) => (
                <div key={idx} className="p-2.5 rounded-lg bg-zinc-900/50 border border-zinc-800/60 space-y-1">
        ...
        <div className="p-4 rounded-xl bg-zinc-950/60 border border-zinc-800/80 space-y-3">
          ...
          {evaluation.gaps.length === 0 ? (
            <p className="text-xs text-zinc-500">Zero critical capability gaps detected for Milestone 1.</p>
          ) : (
            <div className="space-y-2 text-xs">
      ...
      <div className="pt-2">
        ...
        >
          ...
          {showDimensionBreakdown ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
        </button>
        ...
        {showDimensionBreakdown && (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 pt-3">
            {evaluation.dimensions.map((dim, idx) => (
              <div key={idx} className="p-3 rounded-xl bg-zinc-950 border border-zinc-800 space-y-1.5 text-xs">
                <div className="flex items-center justify-between font-semibold">
                  <span className="text-zinc-200">{dim.name}</span>
                  <span className="font-mono text-indigo-400">{dim.score}/{dim.maxScore}</span>
                </div>
