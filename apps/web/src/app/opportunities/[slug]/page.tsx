...
export async function generateMetadata() {
  return {
    title: "Opportunity Blueprint — BuildWorth",
    description: "In-depth 40-attribute startup opportunity blueprint.",
  };
}
...
export default function OpportunityDetailPage() {
  const opp = {
    ...
    oneSentenceSummary:
      "Eliminate quarterly 40-hour screenshot capture sprints for DevOps teams by binding commit signatures to audit controls.",
    opportunityScore: 89,
    confidenceScore: 84,
    ...
    estimatedMvpCost: { minMinor: 500000, maxMinor: 1200000, currency: "USD" as const },
    estimatedTimeToMvpWeeks: { min: 4, max: 8 },
    estimatedMonthlyOperatingCost: { minMinor: 15000, maxMinor: 40000, currency: "USD" as const },
    plausiblePrice: { minMinor: 14900, maxMinor: 39900, currency: "USD" as const },
    ...
    technicalFeasibility:
      "Standard TypeScript, PostgreSQL, GitHub REST/GraphQL API, and Vercel Webhooks. Low research novelty risk.",
    requiredIntegrations: ["GitHub REST & GraphQL API", "Vercel Webhooks", "AWS S3 Evidence Vault"],
    majorAssumptions: [
      "Engineering leaders have credit card discretionary budget up to $500/mo without procurement committee approval",
      "SOC2 auditors accept automated cryptographic commit bundle signatures",
    ],
    majorRisks: [
      "Incumbent compliance platforms add native GitHub Actions integration",
      "GitHub changes enterprise API pricing",
    ],
  };
    ...
    <div className="space-y-8 max-w-4xl mx-auto py-4">
      ...
      <div className="p-8 rounded-2xl bg-zinc-900/60 border border-zinc-800 space-y-6">
        ...
        <div className="space-y-2">
          ...
          <p className="text-base text-zinc-300 leading-relaxed">{opp.oneSentenceSummary}</p>
        </div>
      ...
      <div className="space-y-6">
        ...
        <section className="p-6 rounded-xl bg-zinc-900/40 border border-zinc-800 space-y-4">
          ...
          <div className="space-y-2 text-sm text-zinc-300">
            <div>
              <strong className="text-zinc-200">Existing Workaround:</strong> {opp.existingWorkflow}
            </div>
            <div>
              <strong className="text-zinc-200">Buying Trigger:</strong> {opp.buyingTrigger}
            </div>
          </div>
        ...
        <section className="p-6 rounded-xl bg-zinc-900/40 border border-zinc-800 space-y-4">
          ...
          <ul className="space-y-1.5 text-sm text-zinc-300">
            {opp.narrowMvpScope.map((item, idx) => (
              <li key={idx} className="flex items-start gap-2">
        ...
        <section className="p-6 rounded-xl bg-gradient-to-br from-indigo-950/30 to-zinc-900/40 border border-indigo-500/30 space-y-3">
          ...
          <p className="text-sm text-zinc-200 leading-relaxed">{opp.recommendedNextExperiment}</p>
        </section>
      </div>
    </div>
  );
