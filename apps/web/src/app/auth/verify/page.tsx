  ...
  async function handleConfirmVerification() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(data.error === "TOKEN_EXPIRED" ? "This magic link has expired. Please request a new one." : "This link has already been used or is invalid.");
        setLoading(false);
        return;
      }
  ...
  return (
    <div className="max-w-md w-full bg-slate-900 border border-slate-800 rounded-xl p-8 shadow-2xl text-center">
      ...
      <p className="text-slate-400 text-sm mb-6">
        To protect your security against automated email scanners, please confirm you want to sign in to BuildWorth.
      </p>
      ...
      {error ? (
        <div className="p-4 bg-red-950/40 border border-red-800/60 rounded-lg text-red-300 text-sm mb-6 text-left">
          ...
          <div>{error}</div>
          <div className="mt-3">
            <a href="/onboarding" className="text-xs text-indigo-400 hover:text-indigo-300 underline font-medium">
      ...
      >
        {loading ? (
          ...
          <span>Continue to BuildWorth &rarr;</span>
        )}
      </button>
...
export default function VerifyLandingPage() {
  return (
    <div className="min-h-[70vh] flex items-center justify-center px-4">
      <Suspense fallback={<div className="text-slate-400 text-sm">Loading security verification...</div>}>
        <VerifyContent />
      </Suspense>
    </div>
  );
